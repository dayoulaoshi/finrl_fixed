...
class StockTradingEnv(gym.Env):
    ...
    def _sell_stock(self, index, action):
        def _do_sell_normal():
            ...
            # print(f"here:{self.state[index + 2 * self.stock_dim + 1]}")
            # print(f"here:{len(self.state)}")
            if (
                # self.state[index + 2 * self.stock_dim + 1] != True
                #意义不明（判定是否可以交易？应该是外部信息） 暂时都是True
                True
            ):  
    ...
    def step(self, actions):
        ...
        if self.terminal:
            ...
            if (self.model_name != ""):
                ...
                df_actions.to_csv(
                    "results/actions_{}_{}_{}.csv".format(
                        self.rebalancing_period, self.model_name, self.iteration
                    )
                )
            ...
            temp_day= self.day
            self.day += self.rebalancing_period

            self.data = self.df.loc[temp_day+1:self.day, :]
            if self.turbulence_threshold is not None:
